...
def make_album_part(message: Message, sign: str) \
        -> InputMediaPhoto | InputMediaVideo | InputMediaAudio | InputMediaDocument | None:
    caption = message.html_text + sign if message.caption is not None else sign
...
def make_new_album(messages: list[Message], sign: str) \
    ...
    for message in messages:
        ...
        if ((len(media) > 0) and (not msg_media)) or \
            ...
            new_sign = ""
        item = make_album_part(message, new_sign)
        if item is not None:
            media.append(item)
    return media
