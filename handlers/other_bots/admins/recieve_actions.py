...
async def answer_sender(message: types.Message, session: AsyncSession,
    ...
    await bot.send_message(chat_id=sender_id,
                           text="Администрация ответила на ваше сообщение:")

    if album:
        album.sort(key=lambda m: m.message_id)
        new_album = make_new_album(album, "")
        await bot.send_media_group(chat_id=sender_id,
                                   media=new_album)
        return
...
async def publish(call: types.CallbackQuery,
                  ...
                  state: FSMContext):
    channels = await db.channel_api.get_channels_info(session=session, bot_id=call.bot.id)
    if len(channels) == 0:
...
async def send_to(call: types.CallbackQuery,
                  session: AsyncSession,
                  state: FSMContext):
    ...
    formatting = "\n\n" + db_bot.post_formatting if db_bot.post_formatting is not None \
        else ""
    signature = f"\n\n{html.code('👤 '+sender_name)}" \
    if db_bot.sign_messages else ""
    ...
    if group[0].media_group_id != "":
        media_group = await db.message_api.get_messages_by_media_group_id(
            session, bot.id, call.from_user.id,
            group[0].media_group_id
        )
        ...
        new_album = restore_album(media, sign=signature + formatting,
                                  html_text=group[0].html_text)
    ...
    caption = group[0].html_text + signature + formatting \
    if restored_message.caption is not None else signature
    await restored_message.copy_to(chat_id, caption=caption).as_(bot)

        
@router.callback_query(F.data[:12] == "channel_page")
async def process_channel_page_callback(call: types.CallbackQuery,
                                        session: AsyncSession):
