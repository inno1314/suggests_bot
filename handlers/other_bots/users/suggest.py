...
from utils import make_new_album, generate_id, deserialize_telegram_object_to_python
...
async def send_with_kwargs(
    ...
    await db.message_api.add_message(
        session=session,
...
async def resend_to_admin(
    ...
    for admin_id in admins:
        markup = await make_recive_options(
            message_id=group_id, sender_id=sender_id, sender_username=sender_username
        )
        ...
        try:
            if album:
                ...
                list_of_sent = await bot.send_media_group(
                    chat_id=admin_id, media=new_album
                )
                ...
                kwargs.update(text="———————————————————", reply_markup=markup)
                ...
                await message.answer("<b>🚫 Недопустимый тип вложения</b>")
                return
    ...
    text = current_ad_message.html_text if current_ad_message is not None else None

    if db_bot.answer_message is not None:
        return await message.answer(db_bot.answer_message)
    elif current_ad_message is None:
        await message.answer(text=messages["default_answer"])
    elif current_ad_message.photo_link is None:
        await message.answer(text=text)
    else:
        await message.bot.send_photo(
            chat_id=sender_id, photo=current_ad_message.photo_link, caption=text
        )
