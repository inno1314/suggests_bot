...
async def create_bot(call: types.CallbackQuery,
...
async def add_bot(message: types.Message, state: FSMContext,
    ...
    try:
        ...
        third_msg = await message.answer("<i>Этот токен уже занят.</i>",
                              reply_markup=types.InlineKeyboardMarkup(
                              inline_keyboard=[
                              [types.InlineKeyboardButton(
                              text="OK",
                              callback_data="ok_error")]]))
    ...
    try:
        bot = await db.bot_api.add_bot(session=session, bot_id=bot_user.id,
...
async def ok_added(call: types.CallbackQuery, state: FSMContext,
                   session: AsyncSession):
    data = await state.get_data()
    sent_messages: list = data.get("msgs")
    chat_id = call.from_user.id
    await main_bot.delete_message(chat_id,
                                  message_id=sent_messages[1])
    await main_bot.delete_message(chat_id,
                                  message_id=sent_messages[2])
    await state.clear()
    bots = await db.admin_api.get_admins_bots(session=session,
                             admin_id=call.from_user.id)
    markup = await bots_list(bots)
    if not str(call.data) == "ok_error":
        await main_bot.edit_message_reply_markup(chat_id,
