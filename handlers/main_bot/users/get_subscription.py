...
async def get_premium(call: types.CallbackQuery, session: AsyncSession):
    sub = await db.subscription_api.get_subscription(session, admin_id=call.from_user.id)
    end_date = None
    if sub is not None:
    ...
    sub_status = "<i><b>не активна</b></i>" if end_date is None \
...
async def set_plan(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(text=messages['ru']['payment_method'],
                              reply_markup=payment_methods)
    await call.answer()

@router.callback_query(F.data.in_(["yoomoney", "aaio", "crypto_bot"]))
async def set_pay_method(call: types.CallbackQuery, state: FSMContext,
    ...
    print(f"plan: {plan}, method: {method}, price: {price}")

    await process_payment(session=session, call=call, plan=plan,
                          payment_amount=price, payment_type=method)
...
async def clear_sub(call: types.CallbackQuery, session: AsyncSession):
    ...
    if sub is None:
        ...
        return
    logger.info(f"sub starts: {sub.start_date}, ends: {sub.end_date}")
    ...
    if updated_sub is None:
        logger.info("Subscription successfully cleaned!")
    await call.answer()
