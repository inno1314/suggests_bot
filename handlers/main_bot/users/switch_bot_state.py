...
async def switch_bot_state(call: types.CallbackQuery,
                           bot: Bot,
    ...
    if status:
        ...
        await bot.set_webhook(OTHER_BOTS_URL.format(bot_token=token),
                              allowed_updates=['message', 'callback_query', \
                                               'my_chat_member', 'chat_member'])
        logger.info(f"Bot {db_bot.id} was continued from being paused.")
    
    await db.bot_api.update_bot_field(session=session,
                                      bot_id=bot_id,
                                      field_name="is_active", new_value=new_status)
