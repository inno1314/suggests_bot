        ...
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text="📈 Просмотры", callback_data="stats_view:ads"
                ),
                types.InlineKeyboardButton(
                    text="💰 Доходы", callback_data="stats_view:revenue"
                ),
                types.InlineKeyboardButton(
                    text="📊 Активность", callback_data="stats_view:activity"
                ),
            ],
            [
                types.InlineKeyboardButton(
                    text="🔙 Назад", callback_data="to_admins_menu"
                ),
            ],
        ]
    )
...
async def show_analytics(call: types.CallbackQuery, session: AsyncSession):
    ...
    if cached_data:
        ...
        if view == "ads":
            income_from_subs = await db.subscription_api.get_subscription_income(
                session
            )

            # Optimized count query instead of loading 70,000+ objects
            users_for_ad_mailing_count = (
                await session.execute(
                    select(func.count(Sender.id))
                    .join(Bot, Sender.bot_id == Bot.id)
                    .where(
            ...
            for sub in active_subs[:limit_subs]:
                ...
                bot_names = (
                    ", ".join(
                        f"@{bot.url.split('/')[-1]}"
                        if (bot.url and "/" in bot.url)
                        else bot.name
                        for bot in sub.admin.bots
                    )
                    or "нет"
                )
                subs_lines.append(
            ...
            caption = (
                ...
                f"👥 Рассылку увидят <code>{users_for_ad_mailing_count}</code> пользователей\n"
                f"👁️ Показов рекламы в ответном сообщении (за последние 30 дней): <code>{count}</code>\n\n"
                f"🌟 <b>Премиум-подписчики ({len(active_subs)}):</b>\n{subs_text}"
            )

        elif view == "revenue":
            total_revenue = await db.payments_api.get_successful_payments_revenue(
                session
            )
            conv_rate = await db.payments_api.get_payment_conversion_rate(session)
            revenue_by_service = (
                await db.payments_api.get_successful_payments_by_service(session)
            )
            ...
            latest_payments = await db.payments_api.get_latest_payments(
                session, limit=5
            )
            attempts_lines = []
            ...
            total_suggestions = await db.message_api.get_total_suggestions_count(
                session
            )
            total_bots = (
                await session.execute(select(func.count(Bot.id)))
            ).scalar() or 0

            pre_active_bots = chart_cache.get("pre_rendered_val:active_bots_count")
            pre_top_bots_text = chart_cache.get("pre_rendered_val:top_bots_text")

            if (
                pre_rendered_val is not None
                and pre_active_bots is not None
                and pre_top_bots_text is not None
                and os.path.exists("activity_chart.png")
            ):
                count_30_days = pre_rendered_val
                active_bots = pre_active_bots
                top_bots_text = pre_top_bots_text
                path = "activity_chart.png"
            else:
                path, count_30_days = await generate_activity_chart(session)
                active_bots = await db.message_api.get_active_bots_count(
                    session, days=30
                )
                ...
                stmt = (
                    ...
                    .where(
                        cast(
                            SuggestedMessage.message_data.op("->>")("date"), BigInteger
                        )
                        >= min_timestamp
                    )
                ...
                top_bots_lines = []
                for index, (bot_name, bot_url, count_val) in enumerate(
                    top_bots_data, 1
                ):
                    username = (
                        f"@{bot_url.split('/')[-1]}"
            ...
            total_senders = (
                await session.execute(select(func.count(Sender.id)))
            ).scalar() or 0
            active_senders = (
                await session.execute(
                    select(func.count(Sender.id)).where(Sender.is_active == True)
                )
            ).scalar() or 0
    ...
    if data == "show_incomes":
        ...
        sent_msg = await call.message.answer_photo(
            photo=photo, caption=caption, reply_markup=get_stats_markup()
        )
    else:
