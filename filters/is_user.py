...
class isUser(BaseFilter):
    async def __call__(self, message: Message, session: AsyncSession):
        ...
        if sender is None:
            await db.sender_api.add_sender(
                session=session,
                user_id=user_id,
            ...
            await db.sender_api.add_bot_to_sender(
                session, sender_id=user_id, bot_id=bot_id
            )

        sender = await db.sender_api.get_sender(session, sender_id=user_id)
        admins = await db.bot_api.get_bots_admins(session=session, bot_id=bot_id)
