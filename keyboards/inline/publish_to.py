...
async def channels_list(channels: list[dict],
    ...
    for channel in current_channels:
        new_markup.inline_keyboard.append([InlineKeyboardButton(text=channel['name'],
                                            callback_data=f"send_to {channel['id']}")])
    ...
    if start_index == 0 and len(channels) > end_index:
        new_markup.inline_keyboard.append(
            [InlineKeyboardButton(
                text="Вперед ➡️",
                callback_data=f"channel_page {page + 1}"
                )]
            )
    ...
    new_markup.inline_keyboard.append([InlineKeyboardButton(text="Отмена", callback_data="cancel_publish")])
