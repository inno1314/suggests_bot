...
async def banned_buttons(session: AsyncSession, banlist: list[int],
    ...
    for sender_id in current_users:
        ...
        builder.row(InlineKeyboardButton(text=sender.first_name,
                                         callback_data=f"unban {sender.id}"))
    ...
    if end_index < len(banlist) and start_index > 0:
        builder.add(InlineKeyboardButton(text="Вперед ➡️",
            callback_data=f"page {page + 1}"))
