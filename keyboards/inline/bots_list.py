...
async def bots_list(bots: list[Bot], page: int = 0) -> InlineKeyboardMarkup:
    ...
    if len(bots) <= 10:
        builder.row(InlineKeyboardButton(text="ДОБАВИТЬ НОВОГО БОТА",
    ...
    for bot in current_bots:
        ...
        builder.row(InlineKeyboardButton(text=work_status_emoji + str(bot.name), url=str(bot.url)))
    ...
    if start_index > 0 and end_index < len(bots):
        ...
        builder.row(InlineKeyboardButton(text="Вперед ➡️", callback_data=f"bots_page {page + 1}"))
    elif start_index > 0 and end_index >= len(bots):
        builder.row(InlineKeyboardButton(text="⬅️ Назад", callback_data=f"bots_page {page - 1}"))
